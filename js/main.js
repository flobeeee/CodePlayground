...
class App {
  ...
  loadPage(pageName) {
    try {
      ...
      // 디자인(숨은그림) 페이지인 경우 HiddenPictureGame 초기화
      if (pageName === "design") {
        this.initializeHiddenPictureGame();
      }
  ...
  initializeHiddenPictureGame() {
    // 기존 HiddenPictureGame 인스턴스가 있다면 제거
    if (window.hiddenPictureGame) {
      delete window.hiddenPictureGame;
    }

    // 새로운 HiddenPictureGame 인스턴스 생성
    setTimeout(() => {
      window.hiddenPictureGame = new HiddenPictureGame();
      this.hiddenPictureGame = window.hiddenPictureGame;
    }, 100);
  }
